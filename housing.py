...
class Housing:
    def __init__(self):
        ...
        self.blocking_groups = [] # blocking groups are ints representing size of group
    ...
    def randomly_generate_blocking_groups(self):
        ...
        while students_remaining > 0:
            ...
            if block_size <= students_remaining:
                self.blocking_groups.append(block_size)
            else:
                self.blocking_groups.append(students_remaining)
                block_size = students_remaining

            students_remaining = students_remaining - block_size

        assert(sum(self.blocking_groups) == self.num_students)

        # print(self.blocking_groups)
